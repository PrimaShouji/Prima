...
module.exports = {
	...
	async execute(client, logger) {
		...
		const rsvpList = await readdir(path.join(__dirname, "../../schedules"));

		let finalFilePath;
		let fileName;

		while (fileName = rsvpList.pop()) {
			finalFilePath = path.join(rsvpList, fileName);

			data = (await readFile(finalFilePath)).toString().split(/,/gm);
