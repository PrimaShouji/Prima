...
module.exports = {
    ...
    execute: async (client, logger, message) => {
        ...
        // If someone adds a run tag in as the first argument
        ...
        // Send out notifications that the run was cancelled
        let fileName = found;
        let fileData = (await readFile(`${scheduleFolderPath}/${fileName}`)).toString().split(/,+|\r+\n+/gm).filter((el) => el !== "");
        const fileDay = fileData[0];
        const fileTime = fileData[1];
        fileData = fileData.slice(3)
        while (fileData.length !== 0) {
            const id = fileData.pop();
            try {
                member = client.users.get(id);
                await member.send(`The run you reacted to (hosted by ${message.author.tag} on ${fileDay} at ${fileTime}) has been cancelled.`);
                logger.info(`Cancellation sent to ${member.tag} about ${message.author.tag}'s run.`);
            } catch (e1) {
                logger.error(`${e1}, trying again.`);
                try {
                    member = await client.guilds.get(arsenal_guild_id).fetchMember(id);
                    member.send(`The run you reacted to (hosted by ${message.author.tag} on ${fileDay} at ${fileTime}) has been cancelled.`);
                    logger.info(`Cancellation sent to ${member.tag} about ${message.author.tag}'s run.`);
                } catch (e2) {
                    try {
                        ...
                        client.guilds.find(g => g.id = arsenal_guild_id).channels.find(ch => ch.name === "scheduling-discussion").send(commontags.stripIndents`
                            <@${id}>, the run you reacted to (hosted by ${message.author.tag} on ${fileDay} at ${fileTime}) has been cancelled. (DM failed, fallback hit.)
                        `);
                    } catch (e3) {
