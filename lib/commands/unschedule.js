...
module.exports = {
    ...
    execute: async (client, logger, message) => {
        ...
        // Time processing
        let runTime = message.content.match(/\d+:\d+\s?(?:(?:AM)|(?:PM))/gi)[0].replace(" ", "").trim().toUpperCase();
        if (runTime === "0:00PM") {
        ...
        for (let i = 0; i < dir.length; i++) {
			...
			let data = await readFile(`../../schedules/${dir[i]}`);
        ...
        const data = (await readFile(`../../schedules/${found}`)).toString().split(/,/gm);
        ...
        await unlink(`../../schedules/${fileName}`)
        ...
        if (rows.length) {
            ...
            const messageID = sheetData.data.values[runCoords[0]][runCoords[1]];
