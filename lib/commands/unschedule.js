...
module.exports = {
    ...
    execute: async (client, logger, message) => {
        ...
        const day = fileData[0];
        const time = fileData[1];
        fileData = fileData.slice(3)
        while (fileData.length !== 0) {
            const id = fileData.pop();
            try {
                member = client.users.get(id);
                await member.send(`The run you reacted to (hosted by ${message.author.tag} on ${day} at ${time}) has been cancelled.`);
                ...
                try {
                    ...
                    member.send(`The run you reacted to (hosted by ${message.author.tag} on ${day} at ${time}) has been cancelled.`);
                    logger.info(`Cancellation sent to ${member.tag} about ${message.author.tag}'s run.`);
                } catch (e2) {
                    try {
                        ...
                        client.guilds.find(g => g.id = arsenal_guild_id).channels.find(ch => ch.name === "scheduling-discussion").send(commontags.stripIndents`
                            <@${id}>, the run you reacted to (hosted by ${message.author.tag} on ${day} at ${time}) has been cancelled. (DM failed, fallback hit.)
                        `);
                    } catch (e3) {
