    ...
    execute: async (client, logger, message) => {
        ...
        while (fileData.length !== 0) {
            ...
            try {
                ...
                await member.send(`The run you reacted to (hosted by ${message.author.tag}) has been cancelled.`);
                ...
                try {
                    ...
                    member.send(`The run you reacted to (hosted by ${message.author.tag}) has been cancelled.`);
                    logger.info(`Cancellation sent to ${member.tag} about ${message.author.tag}'s run.`);
                } catch (e2) {
                    try {
                        ...
                        client.guilds.find(g => g.id = arsenal_guild_id).channels.find(ch => ch.name === "scheduling-discussion").send(commontags.stripIndents`
                            <@${id}>, the run you reacted to (hosted by ${message.author.tag}) has been cancelled. (DM failed, fallback hit.)
                        `);
                    } catch (e3) {
