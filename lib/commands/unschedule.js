...
module.exports = {
    ...
    execute: async (client, logger, message) => {
        ...
        let fileName = found;
        const fileData = (await readFile(`${scheduleFolderPath}/${fileName}`)).toString().split(/,+|\r+\n+/gm).filter((el) => el !== "")
        fileData = fileData.slice(3)
        while (data.length !== 0) {
            const id = data.pop()
            try {
                member = client.users.get(id)
                await member.send(`The run you reacted to (hosted by ${leader.tag}) has been cancelled.`)
                logger.info(`Cancellation sent to ${member.tag} about ${leader.tag}'s run.`)
            } catch (e1) {
                logger.error(`${e1}, trying again.`)
                try {
                    member = await client.guilds.get(arsenal_guild_id).fetchMember(id)
                    member.send(`The run you reacted to (hosted by ${leader.tag}) has been cancelled.`)
                    logger.info(`Cancellation sent to ${member.tag} about ${leader.tag}'s run.`)
                } catch (e2) {
                    try {
                        ...
                        client.guilds.find(g => g.id = arsenal_guild_id).channels.find(ch => ch.name === "scheduling-discussion").send(commontags.stripIndents`
                            <@${id}>, the run you reacted to (hosted by ${leader.tag}) has been cancelled. (DM failed, fallback hit.)
                        `)
                    } catch (e3) {
