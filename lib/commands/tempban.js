...
module.exports = {
	...
	execute: async (client, logger, message) => {
        ...
        const collection = (await client.db).db(client.dbName).collection("tempbans");

        const user = getSnowflakeFromMention(message.args[0]);
		if (!user) {
			return await message.author.send("Please specify a user to tempban.");
		}

        const now = new Date();
        let bannedUntil = Date.now();
		...
		bannedUntil += absDateMod + absTimeMod;
