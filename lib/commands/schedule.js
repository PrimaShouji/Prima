...
const runDisplayType = {
    ...
    OZ: { rgb: [249/255, 203/255, 156/255], hex: "#f9cb9c" },
...
module.exports = {
    ...
    execute: async (client, logger, message) => {
        ...
        if (!runDisplayType[runTag]) {
            return message.reply(`${runTag} is not a valid run identifier. Valid identifiers include ${Object.keys(runDisplayType).toString()}`);
        }
        ...
        const hours = parseInt(runTime.match(/\d+:/g)[0].match(/\d+/g)[0]);
        if (hours > 12 || hours < 0) {
            return message.reply(`your time entry was invalid. Please enter a time between 12:00 AM and 11:00 PM.`);
        }
