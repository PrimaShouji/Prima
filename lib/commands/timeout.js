...
module.exports = {
	...
	execute: async (client, logger, message) => {
            ...
            if (day.indexOf("/") !== -1) {
				...
				if (now.getMonth() === mmddyyyy[0] - 1 && now.getFullYear() === yyyy) {
					...
					/*if (yyyy > now.getFullYear()) {
						for (let i = yyyy; i < now.getFullYear() - 1; i++) {
        ...
        if (time) {
            ...
            let curTimeMod = (now.getHours() * 3600000) + (now.getMinutes() * 60000) + (now.getSeconds() * 1000);
		...
		await message.channel.send(moment(timedOutUntil).format("dddd, MMMM Do YYYY, h:mm:ss a"));
		...
		logger.info(`User ${member.user.tag} timed out from ${message.guild.name} until ${timedOutUntil}`);
