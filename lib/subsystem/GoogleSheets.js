    ...
    authorize(credentials) {
        ...
        return new Promise((resolve) => {
            fs.readFile(TOKEN_PATH, (err, token) => {
                if (err) this.getNewToken(oAuth2Client, (newOAuth2Client) => {
                    console.log("Token created!");
                    this.sheets = google.sheets({version: "v4", newOAuth2Client});
                    resolve();
                });
                oAuth2Client.setCredentials(JSON.parse(token));
                this.sheets = google.sheets({version: "v4", oAuth2Client});
                resolve();
            });
        });
    }
