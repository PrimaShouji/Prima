...
client.BAqueues = new Discord.Collection();
BAqueues = client.BAqueues;

for (channel of lfg_channels) {
	dpsHandoff = fs.readFileSync(`queues/${channel}/dps.queue`).toString().match(/\d+/g);
	healerHandoff = fs.readFileSync(`queues/${channel}/healer.queue`).toString().match(/\d+/g);
	tankHandoff = fs.readFileSync(`queues/${channel}/tank.queue`).toString().match(/\d+/g);
	BAqueues.set(`${channel}.dps`, dpsHandoff ? dpsHandoff : []); // Set the loaded queue, or create a new array if nobody's in the queue.
	BAqueues.set(`${channel}.healer`, healerHandoff ? healerHandoff : []);
	BAqueues.set(`${channel}.tank`, tankHandoff ? tankHandoff : []);
}
